...
Minimap.mapColor = tocolor(radarSettings['mapColor'][1]*radarSettings.mapColorScale,radarSettings['mapColor'][2]*radarSettings.mapColorScale,radarSettings['mapColor'][3]*radarSettings.mapColorScale,radarSettings['alpha']);
	...
	if getElementDimension(localPlayer) > 0 then return end
		if (not Minimap.IsVisible and Bigmap.IsVisible) then
			...
			if (getElementInterior(localPlayer) == 0) then
				...
				dxDrawImageSection(Bigmap.PosX, Bigmap.PosY, Bigmap.Width, Bigmap.Height, mapX, mapY, mapWidth, mapHeight, Minimap.MapTexture, 0, 0, 0, Minimap.mapColor);
				...
				for _, blip in ipairs(getElementsByType('blip')) do
					...
					if (localPlayer ~= getElementAttachedTo(blip)) and (getElementType(getElementAttachedTo(blip)) ~= 'player')  then
						local blipSettings = {
		...
		if getElementData(localPlayer,"radar") then return end
			...
			if (getElementInterior(localPlayer) == 0) then
				...
				dxDrawImageSection(0, 0, Minimap.BiggerTargetSize, Minimap.BiggerTargetSize, mapX, mapY, mapWidth, mapHeight, Minimap.MapTexture, 0, 0, 0, Minimap.mapColor, false);
				...
				for _, blip in ipairs(serverBlips) do
					...
					if (localPlayer ~= getElementAttachedTo(blip) and getElementInterior(localPlayer) == getElementInterior(blip) and getElementDimension(localPlayer) == getElementDimension(blip)) and (getElementType(getElementAttachedTo(blip)) ~= 'player') then
